package HTTP;//
// Test program for HTTP.WebServer
...
public final class WebServerTest
{
    ...
    if (argv.length > 1) {
        ...
        if (argv[c].equals("-p")) {
            ...
            System.out.println("usage: HTTP.WebServerTest [-p <port>] " +
                       "[-h <host>]");
            System.exit(0);
        }
        }
    } else if (argv.length != 0) {
        System.out.println("usage: HTTP.WebServerTest [-p <port>] [-h <host>]");
        System.exit(0);
    }
...
final class Tests
{
    ...
    String request =
        ...
        "User-Agent: HTTP.WebServerTest/1.0" + CRLF +
        CRLF;
...
final class HTTPRequest
{
    ...
    request =
        Tests.makeRequestLine(method, resource, Tests.HTTPVERSION,
                  Tests.CRLF) +
        "User-Agent: HTTP.WebServerTest/1.0" + Tests.CRLF +
        Tests.CRLF;
    }
